                ...
                RageUI.IsVisible(MenuPrincipal, function()
                    ...
                    for k, v in pairs(Config.Shop) do 
                        RageUI.Button(v.name, nil, {RightLabel = "~r~"..ESX.Math.GroupDigits(v.price).."$"}, true, {
                            onSelected = function()
                                TriggerServerEvent("sPeche:AchatsItems", v.item, v.name, v.price, true)
                            end,
                        })    
                    end
                        ...
                        RageUI.Button(v.label, nil, {RightLabel = "~r~"..ESX.Math.GroupDigits(v.price).."$"}, true, {
                            onSelected = function()
                                if SpawnVehicleVerif == false then
                                    TriggerServerEvent("sPeche:LouerBateau", v.label, v.price)
                ...
                RageUI.IsVisible(MenuPrincipal, function()
                    ...
                    RageUI.Button("Vendre l'intégralité de vos Poissons", nil, {RightBadge = RageUI.BadgeStyle.Tick}, true, {
                        ...
                        for k, v in pairs(Config.Vente) do 
                            TriggerServerEvent("sPeche:VentePoissons", v.item, v.name, v.price)
                        end
                    end,
                    })    
                end)
...
function Peche()
    ...
    local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
    ESX.TriggerServerCallback('sPeche:AssezCAP', function(hasEnoughCAP) 
        if hasEnoughCAP then  
            ESX.TriggerServerCallback('sPeche:AssezAppats', function(hasEnoughAppats)
...
function RandomFish()
	...
	while Active do
		...
		for k,v in pairs(Config.Random_Poisson) do
			...
			if random == id then
				Active = false
                TriggerServerEvent('sPeche:UtilisationAppat', xfishbait, number)
                TriggerServerEvent('sPeche:RandomPoisson', item, count, name)
                FreezeEntityPosition(PlayerPedId(), false)
            ...
            while MenuOuvert do 
                RageUI.IsVisible(MenuPrincipal,function() 
                    ...
                    RageUI.Button("Rendre le Bateau de Location", nil, {RightBadge = RageUI.BadgeStyle.Tick}, true , {
                        onSelected = function()
                            TriggerServerEvent("sPeche:RetourLocation", price_caution)
        ...
        Citizen.Wait(5000)
		    if SpawnVehicleVerif == true then
			    if GetVehicleEngineHealth(Boat_Vehicle) < 5 and Boat_Vehicle ~= nil then
                    ...
                    TriggerServerEvent("sPeche:BateauDetruit", price_bateau_detruit)
			    end
		    end
	    end
end)
